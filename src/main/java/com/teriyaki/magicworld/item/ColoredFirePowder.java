...
public class ColoredFirePowder extends FirePowder {
    private final DyeColor POWDER_COLOR;

    public ColoredFirePowder(DyeColor color) {
        super();
        this.POWDER_COLOR = color;
    }
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (CampfireBlock.canBeLit(blockstate)) {
            ...
            if (AbstractFireBlock.canLightBlock(world, blockpos, context.getPlacementHorizontalFacing())) {
                this.playUseSound(world, blockpos);
                BlockState coloredFireState = ColoredFireBlock.getFireByColor(this.POWDER_COLOR).getStateForFirePowderUse(world, blockpos, this.POWDER_COLOR);
                world.setBlockState(blockpos, coloredFireState);
                flag = true;
            }
        }
    ...
    // Copied from: FireChargeItem class
